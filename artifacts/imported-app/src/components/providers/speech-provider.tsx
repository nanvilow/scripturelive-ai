  ...
  useEffect(() => {
    processCallbackRef.current = async (text: string, confidence: number) => {
      ...
      if (v2 && v2.confidence >= 80) {
        ...
        //
        // v0.7.247 — RE-MENTION AUTO-LIVE FIRE + EARLY RETURN.
        // Operator escalation: "say John 3:16 → auto-sends; say
        // Habakkuk 1:1 → auto-sends; re-say Habakkuk 1:1 → does NOT
        // auto-send Habakkuk, instead auto-sends whatever's in the
        // Bible Reference Quoted column (Genesis 1:3 from 'let there
        // be light')". Root cause: v0.7.184.2 added the navigator
        // fire on the suppress path BUT did NOT call addDetectedVerse
        // AND did NOT return. Two compounding leaks:
        //   (1) The store-level v0.7.187.2 RE-MENTION AUTO-LIVE
        //       promotion path (store.ts L1085-1091 — fresh id +
        //       fresh detectedAt + front-of-array) is GATED on
        //       addDetectedVerse being called. With the v2 dispatcher
        //       short-circuiting via `requestNavigatorRef` only, the
        //       store reducer never runs → detectedVerses array
        //       reference never changes → the centralized auto-fire
        //       useEffect in logos-shell.tsx (watching detectedVerses)
        //       never re-runs → no auto-go-live on the re-mention.
        //   (2) Without `return`, execution falls through to the
        //       preacher-phrase + keyword text-search + semantic
        //       (`fetchSemanticMatches`) blocks below. Any one of
        //       them firing on the same transcript chunk (typically
        //       semantic matching nearby preacher exposition) lands
        //       its hit in the Bible Reference Quoted column at high
        //       enough confidence to clear the auto-live stability
        //       gate. The operator sees the WRONG verse go live
        //       moments after re-speaking the address.
        //
        // Fix: on the re-mention branch, find the existing
        // DetectedVerse in the store and call addDetectedVerse with
        // a synthetic 'explicit' entry carrying the same refKey. The
        // store's v0.7.187.2 promotion path then preserves the
        // authoritative text/translation/confidence from the existing
        // entry while producing a fresh id + detectedAt + front-of-
        // array — exactly the original-mention shape that the auto-
        // fire useEffect expects. Then RETURN so the semantic /
        // preacher-phrase pipelines below cannot hijack the live
        // display with a co-incidental match.
        if (lastAt > 0 && now - lastAt < REF_DEDUPE_TTL_MS) {
          try { useAppStore.getState().requestNavigatorRef(refKey) } catch { /* defensive */ }
          // Find the existing column entry for this ref so we can
          // re-trigger the store's RE-MENTION AUTO-LIVE promotion
          // path. If for some reason it isn't there (e.g. operator
          // wiped the column manually between mentions), the
          // promotion branch is skipped — addDetectedVerse will fall
          // through to the new-entry insert with our synthetic data,
          ...
          const reAdd: DetectedVerse = existing
            ? {
                ...existing,
                // The store reducer keys off `reference`; id/detectedAt
                // it overwrites in the promotion branch. Pass through
                // existing values otherwise so the dedupe key matches.
              }
            : {
          ...
          // Skip the degraded-no-text case — projector would render
          // an empty slide. Only the happy path (existing found)
          // re-fires auto-live; if existing is missing, the original
          // navigator fire is the only re-mention behaviour, matching
          // pre-v0.7.247 (degraded-but-safe).
          if (existing) {
            useAppStore.getState().addDetectedVerse(reAdd)
            useAppStore.getState().setDetectionStatus('detected')
          }
          // Re-stamp the dedupe timestamp so a third quick re-mention
          // also re-fires (without this, the TTL would carry over
          // from the original fire's timestamp and the next re-say
          // could fall outside the window mid-utterance).
