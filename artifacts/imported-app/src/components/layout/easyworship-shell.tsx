...
export function TransportBar({
  ...
  // v0.7.258 — Transport bar compressed (h-14 → h-9, button h-10 → h-7)
  // per operator request to reclaim vertical space for the Chapter
  // Navigator / Detected Verses / Media row below the stage. The 20px
  // claw-back flows directly to the bottom ResizablePanel above (its
  // defaultSize={48} flex-grows into the freed space). Hotkey kbd
  // hint kept — operators rely on Enter to fire Go/Stop Live.
  return (
    <div className="flex items-center justify-between gap-3 px-3 h-9 border-t border-border bg-background shrink-0">
      <div className="flex items-center gap-1.5">
        <Button
          onClick={onGoLiveToggle}
          disabled={!slides.length}
          className={cn(
            'h-7 px-3.5 font-bold text-[11px] uppercase tracking-wider gap-1.5 shadow-lg',
            isLive
        ...
        >
          {isLive ? <Square className="h-3 w-3" /> : <Send className="h-3 w-3" />}
          {isLive ? 'Stop Live' : 'Go Live'}
          <kbd className="ml-0.5 px-1 py-0.5 rounded bg-black/30 text-[8px] font-mono normal-case tracking-normal leading-none">
            ⏎
          </kbd>
        </Button>
        ...
        <Button
          ...
          variant="ghost"
          className="h-7 px-2.5 text-[11px] uppercase tracking-wider gap-1 text-foreground hover:bg-muted hover:text-foreground border border-border"
        >
