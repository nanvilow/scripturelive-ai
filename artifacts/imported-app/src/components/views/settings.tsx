    ...
    <div className="p-4 md:p-6 lg:p-8 max-w-4xl mx-auto grid grid-cols-1 gap-6 items-start">
      ...
      <Card className="bg-card border-border">
        ...
        <CardContent>
          ...
          {/* v0.7.182 — In-app Display Mode picker REMOVED.
              Operator: "remove the lower third from the app, leaving
              only the lower third that can be operated in NDI settings
              only." The picker previously offered Full Screen / Lower
              Third / Lower Third (Black). With LT now NDI-only, the
              picker collapsed to a single 'Full Screen' option, so the
              entire control was removed (no point in a 1-button radio).
              The persisted `settings.displayMode` is locked to 'full'
              by the type narrowing in store.ts plus the v4→v5 persist
              migration. NDI's separate `ndiDisplayMode` controls the
              broadcast surface and lives in the NDI Output panel. */}
