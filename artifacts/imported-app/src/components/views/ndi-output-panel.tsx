...
export function NdiOutputPanel() {
  ...
  // v0.7.252 — Operator-tunable NDI capture resolution. Default flipped
  // from 1080p → 720p (safer for the long-tail mid-range/older sanctuary
  // PCs that make up most of the install base; vMix/OBS/Wirecast upscale
  // 720p → 1080p in scene composers for free). Operators on high-end
  // hardware opt UP to 1080p via the dropdown below. The `?? '720p'`
  // fallback covers existing-install state where the field is undefined
  // after persist hydration (pre-v0.7.194-hotfix.3 upgrades).
  // Operators who previously CHOSE '1080p' keep it via Zustand persist.
  const ndiCaptureResolution = useAppStore((s) => s.settings.ndiCaptureResolution) ?? '720p'
