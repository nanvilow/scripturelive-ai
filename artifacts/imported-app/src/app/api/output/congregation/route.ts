...
/* v0.5.32 — soft cross-fade. The new content is painted FIRST, then
   this animation eases its opacity from 0.15 to 1 over the operator's
   ...
   v0.7.239 — easing switched ease-out → cubic-bezier(.4,0,.2,1) (Material
   "standard" curve) + will-change:opacity + translateZ(0) GPU layer hint
   for noticeably smoother perceived crossfade. From-opacity dropped
   .25 → .15 to extend the perceived blend without re-introducing the
   v0.5.32 fade-to-black gap. */
#output.soft-in{animation:softIn var(--slide-fade-ms,350ms) cubic-bezier(0.4,0,0.2,1) both;will-change:opacity;transform:translateZ(0)}
@keyframes softIn{from{opacity:.15}to{opacity:1}}
...
function applyRender(s){
  ...
  var style=(s&&s.settings&&s.settings.slideTransitionStyle)||'cut';
...
function render(s){
  ...
  var NDI_DEFAULTS = {
    ...
    // v0.7.239 — default lowered 'xl' → 'lg'. 'xl' caps at 10rem title /
    // 7rem text on a 1920x1080 capture which fills 90%+ of the frame
    // and reads as oversized when receivers (Wirecast, vMix, OBS)
    // composite the NDI feed into a larger program canvas. 'lg' (7.6vw
    // title / 5.2vw text) leaves comfortable breathing room around the
    // bg image. Operator can still pick xl explicitly via the new NDI
    // Text Size picker in the NDI Output panel — that selection writes
    // to settings.ndiFontSize which is honoured at L1694 below.
    fontSize: 'lg',
    textShadow: true,
    textScale: 1.0,
    textAlign: 'center',
  ...
  if(!slide){
    ...
    // v0.7.247 — When the operator picks FULL SCREEN + TRANSPARENT in
    // NDI settings (fsbg=transparent in the URL), the customBackground
    // MUST also be stripped. Pre-fix, FS_BG_TRANSPARENT only stripped
    // theme/bg in the lower-third branch (L2330-2334); the full-screen
    // empty-slide branch (this block) and full-content branch (L1746)
    // ignored it, so a sermon-clip customBackground kept mounting in
    // #bgLayer and vMix/OBS/Wirecast received text-over-clip instead of
    // text-on-alpha-matte. Gating safeBgE here mirrors the LT branch.
    if (FS_BG_TRANSPARENT) safeBgE='';
  ...
  // v0.7.247 — Strip customBackground when operator picked FULL SCREEN
  // + TRANSPARENT (see ~L1688 for the empty-slide branch's matching
  // gate). MUST run BEFORE bg/themeBg/setBgVid composition so #bgLayer
  // stays empty and theme gradient also goes transparent (full alpha
  // matte for vMix/OBS/Wirecast). The LT branch (L2330-2334) already
  // honoured FS_BG_TRANSPARENT; this closes the full-screen leak.
  // Gated on !isLT because isLT goes through the LT branch below which
  // has its own FS_BG_TRANSPARENT handling; double-stripping here would
  // be harmless but obscures the source-of-truth.
  if (FS_BG_TRANSPARENT && !isLT) safeBg='';
  ...
  if(slide.type==='media'&&slide.mediaUrl){
      ...
      try{
        ...
        if(typeof slide.mediaCurrentTime==='number'&&slide.mediaCurrentTime>=0){
          ...
          if(__isTransportEvent){
            ...
            try{existingVid.currentTime=slide.mediaCurrentTime;}catch(e){}
            try{if(existingVid.playbackRate!==1.0)existingVid.playbackRate=1.0;}catch(e){}
          }else if(__absDrift>0.10&&!IS_NDI){
            // v0.7.250 — RESTORED !IS_NDI gate (regressed since v0.7.235).
            // canonical replit.md v0.7.235 guard-rail: "SOFT-TRIM
            // playbackRate 0.97/1.03/1.0 on routine drift !IS_NDI only".
            // Pre-v0.7.250 the gate was dropped and the soft-trim fired
            // on NDI/Wirecast + OBS Browser Source (?ndi=1 → IS_NDI=true)
            // + second-screen popup every SSE tick (~20Hz) whenever drift
            // exceeded 100ms. That hammered playbackRate=0.97/1.03 writes
            // at 20Hz on every secondary surface, which (a) retunes the
            // audio sample rate on every write → audible pitch stutter
            // on the Wirecast/OBS audio bus, (b) forces NV12 decoder
            // frame-timing recalc → choppy video, (c) prevents the
            // Chromium media pipeline from settling into steady state
            // → constant CPU spike. Operator complaint v0.7.249:
            // "video lags, smoothness is very bad, video play slow"
            // on all three secondary surfaces simultaneously. NDI/OBS
            // are now fully exempt from playbackRate trim (consistent
            // with the v0.7.234 hard-snap exemption — both bug classes
            // are the same root cause: secondary decoders cannot tolerate
            // mid-stream rate/seek mutations).
            //
            // Hysteresis: only WRITE if rate actually changes. Same value
            // re-writes still trigger Chromium's media-pipeline rate apply
            // path (audio resampler + frame timer recalc). Reading the
            // current rate and gating the write is free and eliminates
            // 95%+ of churn on the popup (steady-state drift sits at the
            // same +0.03 / -0.03 trim for many seconds at a time).
            //
            // GUARD-RAIL: any future change to either branch MUST update
            // both — the !IS_NDI gate is load-bearing for Wirecast/OBS
            // smoothness, and the hysteresis guard is load-bearing for
            // popup CPU. Test on a connected NDI receiver + OBS Browser
            // Source, NOT just the in-app popup.
            try{var __tgt=__drift>0?0.97:1.03;if(existingVid.playbackRate!==__tgt)existingVid.playbackRate=__tgt;}catch(e){}
          }else{
            // Inside dead band (or NDI/OBS at any drift level) — restore
            // nominal rate so a previous trim eventually ends (otherwise
            // playbackRate stays trimmed and the receiver overshoots the
            // other way). NDI/OBS were never trimmed under the v0.7.250
            // restored gate so the !==1.0 hysteresis check no-ops on
            // them after the first frame and costs nothing.
            try{if(existingVid.playbackRate!==1.0)existingVid.playbackRate=1.0;}catch(e){}
          }
        }
        __prevMediaPaused=slide.mediaPaused;
        ...
        // v0.7.253 — Mirror the operator's Loop button onto the live
        // <video> element WITHOUT rebuilding the DOM, so toggling Loop
        // OFF mid-playback takes effect on the very next SSE tick on
        // every secondary surface (secondary screen, NDI, OBS Browser
        // Source, Wirecast receiver). Hysteresis (value-change gate)
        // matches the v0.7.250 playbackRate pattern: Chromium re-walks
        // the media pipeline on every .loop write regardless of value
        // change, so guarding the write is free CPU on the 20Hz tick.
        // Default false in store ⇒ fresh clips no longer loop forever.
        try{var __loop=!!slide.mediaLoop;if(existingVid.loop!==__loop)existingVid.loop=__loop;}catch(e){}
    ...
    var mediaTag=slide.mediaKind==='video'
      ? '<video id="liveVideo" src="'+__slMU+'" '+(slide.mediaPaused?'':'autoplay ')+(slide.mediaLoop?'loop ':'')+'muted playsinline preload="auto" style="'+mediaStyle+'"></video>'
  ...
  if(isLT){
    ...
    // v0.7.250 — Wirecast-parity for OBS Browser Source LT mode. Added
    // (isLT && IS_NDI) to the transparent-surround gate so OBS Browser
    // Source URL always renders transparent surround in LT mode, exactly
    // like Wirecast does via NDI. Pre-fix the gate was URL-flag dependent
    // (FORCE_TRANSPARENT === ?transparent=1), but the URL builder bakes
    // that flag at COPY TIME — operators who copied the URL in Full mode
    // and later switched to LT were stuck with solid #000 around the bar
    // forever (operator screenshot: BLACK top half + LT bar at bottom in
    // OBS, while Wirecast on the same source showed clean transparency).
    // The new IS_NDI gate auto-heals every NDI surface (FrameCapture for
    // NDI Tools/Wirecast, OBS Browser Source, NdiPreviewSurface in the
    // settings panel) the moment SSE delivers ndiDisplayMode lower-third,
    // regardless of what params are baked into the URL. Non-NDI surfaces
    // (in-room projector, secondary-screen popup — both IS_NDI=false) are
    // intentionally unchanged: they keep solid #000 around the bar because
    // their browser windows are OPAQUE and transparent body would expose
    // Chromium default white window background.
    // (NOTE: this whole block is inside the const html = ... template
    // literal opened at L21 — backticks are forbidden in inline comments
    // here or they will close the outer string and corrupt the served JS;
    // same hazard as the documented </S> and \s+ escapes elsewhere.)
    try{
      var __bg=(FORCE_TRANSPARENT||ltTransparent||(isLT && IS_NDI))?'transparent':'#000';
