...
// Perf: precompile the word-boundary RegExp for each alias once at
// module load. findTranslationAlias runs on every transcript frame;
// rebuilding ~60 RegExp objects per call was pure CPU churn. These
// patterns carry no 'g' flag so .test() is stateless — the compiled
// list is behaviour-identical to the per-call construction it replaces,
// and preserves the longest-first ordering of TRANSLATION_ALIAS_KEYS.
const TRANSLATION_ALIAS_REGEX: Array<{ code: string; alias: string; re: RegExp }> =
  TRANSLATION_ALIAS_KEYS.map((alias) => ({
    code: TRANSLATION_ALIASES[alias]!,
    alias,
    re: new RegExp(`(?:^|\\s)${alias.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}(?:\\s|$)`),
  }))
