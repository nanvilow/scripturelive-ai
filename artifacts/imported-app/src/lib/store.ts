...
export const useAppStore = create<AppState>()(
  persist(
    (set, get) => ({
      ...
      verseHistory: [],
      // v0.7.264 — Consecutive-duplicate suppression. The speech
      // pipeline calls addToVerseHistory on every re-emitted final
      // hypothesis (and a speaker often repeats a reference), which
      // pre-fix pushed N identical rows in a row (operator screenshot
      // 2026-05-30: "Habakkuk 1:1 MSG" ×4). Collapse to ONE entry per
      // verse when the incoming verse matches the most-recent entry by
      // reference+translation. A DIFFERENT verse in between still
      // breaks the run, so the same reference can legitimately reappear
      // later in the service (matches addDetectedVerse semantics).
      addToVerseHistory: (v) =>
        set((state) => {
          const last = state.verseHistory[0]
          if (
            last &&
            last.reference === v.reference &&
            (last.translation ?? '') === (v.translation ?? '')
          ) {
            return {}
          }
          return { verseHistory: [v, ...state.verseHistory].slice(0, 50) }
        }),
      ...
      // v0.7.258 — Default flipped true → false per operator request.
      // Fresh installs now show the FULL raw transcript ("ALL" button
      // un-highlighted) instead of the curated Bible-only view. The
      // amber "Bible" pill remains available — operators who want the
      // filter just click it on. Existing operators who explicitly
      // toggled the filter keep their persisted choice through Zustand
      // persist. Same pattern as v0.7.252 / v0.7.257 default flips.
      bibleOnlyTranscription: false,
      ...
      addScheduleItemQuiet: (item) => {
        // v0.7.264 — Consecutive-duplicate suppression for the quiet
        // append path (auto-fire detection trail + single-click append).
        // Auto-live re-fires the same reference frame after frame as the
        // preacher keeps reading it, and the v0.7.187.2 re-mention
        // promotion mints a fresh id each time, so the queue grew with
        // identical rows (operator screenshot 2026-05-30: "Romans 8:28"
        // ×3 back-to-back). Collapse when the LAST queue item is an
        // identical entry (same type+title+subtitle); a different item in
        // between still breaks the run. addScheduleItem (the explicit
        // manual Go-Live path) is intentionally NOT deduped — manual
        // re-queueing of the same verse/song is operator intent.
        const existing = get().schedule
        const last = existing[existing.length - 1]
        if (
          last &&
          last.type === item.type &&
          last.title === item.title &&
          (last.subtitle ?? '') === (item.subtitle ?? '')
        ) {
          return last.id
        }
        const id = `sch-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`
