 ...
 */
function hydrateDesktopUpdateToastFromDisk() {
    const prefs = loadAppPreferences();
 ...
 */
function setDesktopUpdateToastEnabledAndPersist(next) {
    const prefs = loadAppPreferences();
...
async function createMainWindow(url, opts = {}) {
 ...
 */
async function prepareTrayBadges(baseIconPath, size) {
    const colors = ['blue', 'orange', 'green', 'red'];
    try {
        const renders = await Promise.all(colors.map(c => (0, tray_badges_1.renderBadgedIcon)(baseIconPath, size, c)));
 ...
 * by accident.
 */
async function confirmInstallDuringBroadcast(version) {
    const choice = await showDialog({
        type: 'warning',
        title: 'Install update during broadcast?',
        message: `Install v${version} now and drop the NDI feed?`,
        detail: 'Restarting will drop the NDI feed for about 10 seconds while ' +
...
function setupIpc() {
    ...
    electron_1.ipcMain.handle('app:get-desktop-update-toast', () => ({
        value: desktopUpdateToastEnabled,
    }));
    electron_1.ipcMain.handle('app:set-desktop-update-toast', (_e, value) => {
        const next = value === true;
        try {
...
if (!gotSingleInstanceLock) {
    ...
    electron_1.app.on('second-instance', (_event, _argv, _workingDirectory) => {
...
electron_1.app.whenReady().then(async () => {
    ...
    if (!bootHidden && process.platform === 'win32') {
        try {
            bootHidden = electron_1.app.getLoginItemSettings().wasOpenedAsHidden === true;
        }
    ...
    try {
        ...
        if (bootHidden)
            console.log('[boot] launched hidden — main window created with show:false, tray-only UI');
    }
    catch (err) {
