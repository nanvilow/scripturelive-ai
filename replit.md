...
-   **Cross-Machine Activation + Payment Cloud Sync (v0.7.145)** — full text in `artifacts/imported-app/CHANGELOG.md`. Still relevant: `src/lib/licensing/cloud-sync.ts` (`cloudClaimActivation` + `cloudMirrorPayment`, both no-op when `process.env.REPLIT_DEPLOYMENT_ID` is set so cloud doesn't talk to itself, both respect `SCRIPTURELIVE_CLOUD_BASE` env override defaulting to `https://scripturelive.replit.app`); cloud routes `/api/license/cloud/{claim-activation,mirror-payment}` (atomic claim locks code to `installId` via `CLOUD-CLAIMED:<installId>` prefix stashed in `lastSeenLocation` — NO schema migration); `activate/route.ts` falls back to cloud ONLY when local `activateCode()` throws `/not recognis|not found|unknown|invalid/i` AND code isn't a master code; `payment-code/route.ts` fires `cloudMirrorPayment` fire-and-forget after every successful `createPaymentCode`. Cloud (scripturelive.replit.app) MUST be redeployed from each release before customers update or first activation will 404. Concurrency caveat: `claimActivationForCustomer` reads from in-memory cache before fire-and-forget `persist()` — sub-millisecond race window where two simultaneous claims could both win; acceptable for current traffic, add `async-mutex` if it becomes an issue.
