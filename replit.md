...
-   **v0.7.159 — NDI Live Preview Background + StableStage Restored + slideOverride Content-Hash**: Operator follow-up to v0.7.158 with screenshots: (a) NDI Live Preview pane in the NDI Output card showed a WHITE void above the lower-third bar instead of the user's configured background image — the bar floated on bare iframe transparency that bled through to the parent card. (b) Architect review of v0.7.158 flagged two minor regressions that needed cleanup: PreviewCard lost its `<StableStage>` wrap (causing iframe text to reflow on column-resize drag), and the OutputPreview `useEffect` only depended on `slideOverride?.id` (so editing the title/body of the queued slide without changing its id wouldn't trigger a re-post). **Fix A (NDI Live Preview background)**: removed the `transparent=1` query param from the iframe URL constructed inside `NdiPreviewSurface` in `src/components/views/ndi-output-panel.tsx`. The actual NDI capture (a hidden FrameCapture BrowserWindow elsewhere in `electron/main.ts`) STILL passes `transparent=1` to keep OBS / vMix alpha clean — only the operator's WYSIWYG preview pane drops it, which makes the renderer paint `customBackground` (image or video) exactly like Settings PREVIEW does. Operator now sees the SAME background behind the lower-third bar in every pane. **Fix B (StableStage restored on PreviewCard)**: `<OutputPreview>` in `logos-shell.tsx`'s PreviewCard is now wrapped in `<StableStage isLive={false}>` again (lost in v0.7.158); column-resize drags no longer reflow the iframe's vw/cqw typography on every tick because the inner stage is frozen at a fixed pixel viewport and CSS-scaled. **Fix C (content-hash dep)**: `useEffect` dep array in `output-preview.tsx` line 270 changed from `slideOverride?.id` to `slideOverride ? JSON.stringify(slideOverride) : null` — slides are tiny objects so the stringify cost is negligible and any title/body edit re-flushes the iframe immediately. **Files**: `src/components/views/ndi-output-panel.tsx` (1-line removal of `transparent=1`), `src/components/settings/output-preview.tsx` (1-line dep widen), `src/components/layout/logos-shell.tsx` (PreviewCard's bare `<div>` wrap → `<StableStage>`). Tests: tsc --noEmit clean. `package.json` + `BUILD.bat` banner bumped to 0.7.159. **GUARD-RAIL**: any future iframe surface intended for OPERATOR consumption (preview pane, settings preview, NDI live preview) MUST omit `transparent=1` so the user's background renders. Only ACTUAL CAPTURE surfaces (FrameCapture BrowserWindow, OBS Browser Source URL pasted into OBS) need transparency for downstream chroma keying. The `?ndi=1` flag alone is sufficient to opt into NDI-specific layout (max-width cap, font clamps); transparency is orthogonal.
