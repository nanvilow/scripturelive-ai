...
-   **UI/UX:**
    ...
    -   **Production-deployment requirement (desktop):** the Windows Electron build's `DEFAULT_TRANSCRIBE_PROXY_URL` (`https://scripturelive.replit.app/api/transcribe`) must point at a host that hosts the api-server's WS upgrade handler. The `scripturelive.replit.app` autoscale Next.js deployment does NOT host that handler; until the api-server is deployed (Reserved VM with WebSocket support) at a reachable URL, set `TRANSCRIBE_PROXY_URL` (Electron env) or `TRANSCRIBE_STREAM_WSS_URL` to a working api-server endpoint, otherwise the desktop will continue to surface the renderer's improved 1006 explanation.
