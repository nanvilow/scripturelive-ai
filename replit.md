...
-   **Hide "DG" Engine Badge from Live Transcription Card (v0.7.141)** — archived to `artifacts/imported-app/CHANGELOG.md`. **Superseded by v0.7.144** which reverted the full-badge `<div hidden>` wrapper because it also killed the pulsing live-mic indicator inside the pill. Current badge is a single `<span>` rendering JUST the pulsing dot (no text, no dropdown). DropdownMenu / engineLabel / engineTitle still defined above the JSX and intentionally unused — keep them so we never lose the picker as a support escalation lever.
-   **Single Update-Available Modal + HTML Strip in "What's new" (v0.7.140)** — archived to `artifacts/imported-app/CHANGELOG.md`. Still relevant: single canonical update-modal mount point = `src/app/layout.tsx` (the duplicate `<UpdateAvailableDialog>` mount in `src/app/page.tsx` from `@/components/providers/update-dialog` was deleted; do NOT re-introduce it). `cleanReleaseNotes()` in `src/lib/release-notes.ts` strips raw HTML in a three-pass anchor→block→remaining-tag sweep so GitHub release notes authored in HTML render as readable prose; `rehype-raw` deliberately NOT enabled (security + closes the github.com link surface).
-   **Installer Polish — Finish Page, Header Logo, Rewritten "You agree NOT to" (v0.7.139)** — archived to `artifacts/imported-app/CHANGELOG.md`. Still relevant: `customFinishPage` macro in `artifacts/imported-app/build-resources/installer.nsh` defines `MUI_FINISHPAGE_TITLE="Installation Complete"` + run-checkbox copy. v0.7.142 layered an additional `customInstall` MessageBox on top so the auto-updater silent flow also gets a notification surface. `installerHeader.bmp` is BMP3 24-bit 150×57 (only format MUI2 reliably reads); `license.txt` section 3 uses `(a)…(f)` ASCII enumeration (UTF-8 bullets render as mojibake in NSIS).
