...
-   **v0.7.160 — Cross-Device Admin Sync Made Visible + Auto-Pull On Config Read**: Operator complaint: "actions on my phone admin panel don't show up on my desktop and vice versa — implement it well." The transport added in v0.7.153 (cloudPullAdminLedger / cloudPushAdminLedger / applyAdminLedgerSnapshot) was actually fully wired — `admin/list/route.ts:53` and `admin/codes/route.ts:36` pull-and-merge on every read, and `storage.ts:667` debounces an auto-push after every write — but TWO things made it look broken to the operator: (a) the `admin/config` GET route did NOT pull, so price/momo/notification overrides edited on the cloud never reached desktop until the operator manually re-saved; (b) the only sync-status surface was buried in the Settings tab's "Cross-Device Admin Sync" card (added v0.7.157) — operator never opened that tab and had no idea sync was either already working or silently no-op'd because `cloudAdminCode` was unset. **Fix A (config pull)**: `src/app/api/license/admin/config/route.ts` GET now mirrors the list/codes pattern — `getFile()` → `cloudPullAdminLedger({installId, config, timeoutMs: 4000})` → `applyAdminLedgerSnapshot(snap)`. Added imports for `getFile`, `applyAdminLedgerSnapshot`, `cloudPullAdminLedger`. Failures swallowed (local cache wins); per-PC `cloudAdminCode` + `adminPassword` are stripped on the cloud snapshot side so they cannot overwrite local credentials. **Fix B (header status badge)**: New `probeCloudSync()` callback in `admin-modal.tsx` POSTs `/api/license/admin/cloud-sync-test` and stores the result in the existing `cloudSyncResult` state. New `useEffect` fires it on `(open && authed)` and re-fires every 30 s. The DialogTitle (line 1090) now renders a clickable pill badge with one of four states — `connected (N cloud records)` emerald / `wrong key — set up` rose / `cloud unreachable` amber / `not set up — click here` zinc — that jumps to the Settings tab on click and exposes `cloudSyncResult.detail` as a tooltip. **Fix C (Sync Now button)**: A second header button calls `Promise.all([reload(), probeCloudSync()])` and toasts "Synced with cloud" — gives the operator instant proof the round-trip is working without waiting for the 5 s `reload` poll. Both header controls are gated on `authed` so they don't render before login. **Files**: `src/app/api/license/admin/config/route.ts` (+15 lines: imports + try/catch pull block in GET), `src/components/license/admin-modal.tsx` (+~85 lines: `probeCloudSync` callback + useEffect + header badge + Sync Now button). Tests: `tsc --noEmit` clean. `package.json` + `BUILD.bat` banner bumped to 0.7.160. **GUARD-RAIL**: any future admin-data read endpoint that returns ledger fields (paymentCodes / activationCodes / notifications / config) MUST start with the same 4 s `cloudPullAdminLedger` + `applyAdminLedgerSnapshot` block before reading local state — otherwise it'll silently desync from the cloud. Conversely, any field that is per-PC (admin password, cloud admin code, OS-specific paths) MUST stay on the snapshot extractor's strip-list in `storage.ts` so that pulls don't overwrite the local credential. **The cloud at scripturelive.replit.app must already be running v0.7.153+** (carries the `/api/license/cloud/admin-snapshot` + `/admin-merge` routes) — no NEW cloud routes were added in this release, so no fresh redeploy is required PROVIDED the cloud was redeployed at v0.7.153 or later. Operator still needs to paste the cloud's `masterCode` into Settings → Cross-Device Admin Sync card on each device once — the new header badge now makes the "not set up" state immediately obvious instead of silently invisible.
